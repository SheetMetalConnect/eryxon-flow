...
Location tracking answers one question on the floor: "where did you put it?" — once, at the machine, instead of reconstructing it later. It is on by default and can be turned off per shop.
...
If your floor does not move parts between buffers, turn it off in **Organization Settings → Location tracking** — when off, the terminal does not touch any of it. The onboarding wizard asks the same question when you import the demo data: with drop-off locations on, the demo creates four slots per cell and places the parts that already finished a step, including one full slot, so the picker, the queue and the admin grid show real occupancy from the first minute.

## Admin setup

Lay out your physical drop-off slots at **Configuration → Locations** (Organization Settings reminds you with a link while no slots exist):
...
The same view lives in the operator terminal's **Location** tab, so an operator can check or change where a part is at any time — not only at completion. It shows the current slot (or "not placed yet"), where the part is heading next and how many free slots that cell still has; when they are all full the tab says so in red.

In the work queue every part that sits in a slot carries its slot code next to the part number, so an operator picking the next job sees at a glance where to fetch it.
