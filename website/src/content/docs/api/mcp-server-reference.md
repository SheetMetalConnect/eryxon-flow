...
**MCP server for self-hosted Eryxon Flow MES deployments.** It uses direct Supabase access with a service role key, so run it only on trusted infrastructure.
