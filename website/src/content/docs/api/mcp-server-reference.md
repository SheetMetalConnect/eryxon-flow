...
description: "Tools, resources, prompts, transport and error contract of the Eryxon Flow MCP server (3.0, protocol 2026-07-28)."
---

The MCP server gives an AI agent the same production capabilities as the app. It runs next to a self-hosted Eryxon Flow with the Supabase service-role key, so keep it on trusted infrastructure. Every write uses the database functions the app uses (`transition_operation`, `time_entry_action`, `transition_batch`, `add_batch_operations`), so production rules, sequential release and webhook events behave exactly as in the terminal.

Setup and deployment: [MCP Server Setup](/guides/mcp-setup/). Walk-through: [MCP Demo Guide](/api/mcp-demo-guide/).

## Contract

- Protocol revision **2026-07-28** only, on `@modelcontextprotocol/server` 2.0. Transports: stdio, or stateless Streamable HTTP at `/mcp` (fresh server per request, no session id, bearer authentication). Clients negotiate with `server/discover`; a 2025-era `initialize` is answered `-32022`. The standard headers `MCP-Protocol-Version`, `Mcp-Method` and `Mcp-Name` are validated against the body on every request (`-32020` on mismatch), and every result carries `_meta["io.modelcontextprotocol/serverInfo"]`.
- Every tool declares `title`, a JSON-Schema `inputSchema` and `outputSchema` (generated from zod), and annotations: `readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint` (always false; the server only talks to your database). Results return `structuredContent` and a JSON text block. `tools/list`, `prompts/list` and `resources/list` advertise a one-hour shared cache hint; tools are listed in a stable, name-sorted order.
- Deletes are multi-round-trip requests: the first call returns `input_required` with a confirmation elicitation and a sealed `requestState`; the client answers `confirm: true` and retries, and only then is the row deleted. Tampered or expired state is refused before the handler runs.
- A refused production transition (database SQLSTATE `22023`) is returned as a tool result with `isError: true` and `{"error":{"code":"INVALID_STATE_TRANSITION","message":"<rule text>"}}`, for example `Stop active work before starting another operation` or `Previous operation must be completed first` when sequential release is on. Other codes: `NOT_FOUND`, `DUPLICATE_ENTRY`, `INVALID_REFERENCE`, `FORBIDDEN`, `VALIDATION_ERROR`, `DATABASE_ERROR`, `INTERNAL_ERROR`. Invalid arguments are rejected by the SDK before the handler runs.
- Job and part status and the current cell are derived from the operations; no tool sets them directly.
- Writes the database attributes to a person (`created_by` on issues, `assigned_by` on assignments, `recorded_by` on production reports) take the id from the call or from `MCP_ACTOR_ID`.

## Resources and prompts

| Resource | Content | Cache |
|----------|---------|-------|
| `eryxon://jobs` | Open jobs, earliest due first | 30 s, private |
| `eryxon://jobs/{jobId}` | One job with parts and routing | none |
| `eryxon://cells` | Cells in routing order with WIP limits | 1 h, shared |
| `eryxon://cells/{cellId}/wip` | Open operations at a cell | none |
| `eryxon://timers` | Running timers: who is clocked on what | none |

Prompts: `release-plan-for-job` (argument `job_number`) proposes what to release next respecting sequence and WIP limits; `shift-handover` summarises running work, Yellow Cards and overdue parts.
...
The server's `src/coverage.ts` maps every app and REST capability to these tools and its test suite fails when the map has a gap.
