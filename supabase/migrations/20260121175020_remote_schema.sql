...
CREATE OR REPLACE TRIGGER "notify-new-signup" AFTER INSERT OR DELETE OR UPDATE ON "public"."tenants" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://vatgianzotsurljznsry.supabase.co/functions/v1/notify-new-signup', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZhdGdpYW56b3RzdXJsanpuc3J5Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MjY5MDYwOSwiZXhwIjoyMDc4MjY2NjA5fQ.TRtCgKNm9ld-_PGbidweq-mUqhi7Z_7cMcp3AQY3FQQ"}', '{}', '5000');
...
GRANT ALL ON FUNCTION "public"."accept_invitation"("p_token" "text", "p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."accept_invitation"("p_token" "text", "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."accept_invitation"("p_token" "text", "p_user_id" "uuid") TO "service_role";
