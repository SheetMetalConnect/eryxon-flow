-- A quick terminal completion may finish an operation without first opening a
-- timer. Apply the same sequential-release rule to that path as to start.
...
BEGIN
  ...
  IF p_action IN ('start','finish') AND v_op.status = 'not_started'
  ...
  ELSE
    ...
    END IF;
    IF p_action = 'complete' AND v_op.status = 'not_started' THEN
...
REVOKE ALL ON FUNCTION public.transition_operation(uuid, uuid, text, uuid, text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.transition_operation(uuid, uuid, text, uuid, text)
