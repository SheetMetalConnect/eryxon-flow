...
| **Bullet Card** | Een POLCA-voorrangskaart (Rajan Suri, *The Practitioner's Guide to POLCA*). De job met de kaart staat altijd bovenaan: hij wordt bij elke cel de volgende job en blijft bovenaan tot de kaart wordt verwijderd. Als richtlijn omzeilt hij de vrijgavedatum en de capaciteitspoort van de volgende cel en start hij vóór geautoriseerd werk. Spaarzaam gebruiken: één ideaal, hooguit twee, een richtlijn en geen afgedwongen grens. Geen algemene spoedmarkering. Opgeslagen als `parts.is_bullet_card`. (Sommige operatorschermen tonen dit nog als "Rush", een bekende fout.) |
| **Yellow Card** | Een QRM-wachtkaart op een bewerking. Een bewerking met een Yellow Card verdwijnt uit de capaciteitsweergaven en telt niet mee in de max-per-cel telling (POLCA-OHW), zodat parkeren een OHW-plek vrijmaakt zolang de bewerking wacht. Geen algemene pauzemarkering — de uitsluiting van capaciteit en OHW is het doel. Opgeslagen als `operations.status = 'on_hold'`. |
