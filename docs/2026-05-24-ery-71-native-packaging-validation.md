...
## Native validation lane (CI)

Because the local validation host cannot satisfy the iOS/Android acceptance
checks, native validation runs on a CI lane:

- Selected lane: GitHub Actions macOS 15 (`.github/workflows/native-build-smoke.yml`).
- Scope: pin Node 20 + Java 17 on GitHub's macOS 15 runner, run `npm run ios:init`,
  build the generated iOS workspace for the simulator, then run
  `npm run android:assemble:debug` and `npm run android:assemble:release`, and
  upload logs/artifacts for review.
