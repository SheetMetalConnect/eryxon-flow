...
| **Self-hosted backend** | Build with `VITE_SUPABASE_URL=http://your-host` and either bundle that into the IPA or set `CAPACITOR_SERVER_URL=https://your-host` for live-reload | Same; Android release builds are HTTPS-only, with LAN cleartext limited to debug builds or an explicit per-host custom build (see docs/ANDROID.md) | Build with `VITE_SUPABASE_URL=http://127.0.0.1:54321`; the meta-CSP localhost strip is auto-skipped when the URL is local (see `vite.config.ts:stripDevCspForProd`) |
