...
export const Operations: React.FC = () => {
    ...
    try {
      ...
      if (data) {
        const mappedOps: Operation[] = data.map((op: any) => ({
          ...
          stage: op.parts?.stages?.name || 'Unknown',
  ...
  const filteredOperations = operations.filter((op) => {
    ...
    if (filterStage !== 'all' && op.stage !== filterStage) return false;
    if (filterStatus !== 'all' && op.status !== filterStatus) return false;
    if (filterAssigned !== 'all') {
      if (filterAssigned === 'unassigned' && op.assigned_to !== null) return false;
      if (filterAssigned === 'assigned' && op.assigned_to === null) return false;
    }
    return true;
  });
  ...
  const handleExport = () => {
    const csv = [
      ['Operation', 'Part', 'Job', 'Stage', 'Assigned', 'Status'].join(','),
      ...filteredOperations.map((op) =>
        [op.operation_type, op.part_number, op.job_number, op.stage, op.assigned_name || '-', op.status].join(',')
      ),
    ].join('\n');
  ...
  const uniqueStages = [...new Set(operations.map((op) => op.stage))];
      ...
      <Box sx={{ mb: 3 }}>
        ...
        {selected.size > 0 && (
          ...
          >
            ...
            <Button startIcon={<PersonAddIcon />} onClick={handleBulkAssign} variant="contained" size="small">
              Assign Selected
            </Button>
            <Button onClick={() => setSelected(new Set())} variant="outlined" size="small">
              Clear Selection
            </Button>
          </Box>
        )}
          ...
          <TableHead>
            <TableRow>
              <TableCell padding="checkbox">
                <Checkbox
                  checked={selected.size === filteredOperations.length && filteredOperations.length > 0}
                  indeterminate={selected.size > 0 && selected.size < filteredOperations.length}
